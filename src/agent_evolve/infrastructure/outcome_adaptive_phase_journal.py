"""Durable JSONL interception for outcome-adaptive evolution phases."""
...
DURABLE_OUTCOME_ADAPTIVE_PHASE_JOURNAL_ID = (
    "durable_jsonl_outcome_adaptive_phase_journal"
)
DURABLE_OUTCOME_ADAPTIVE_PHASE_JOURNAL_VERSION = 1
DURABLE_OUTCOME_ADAPTIVE_PHASE_JOURNAL_DEFINITION_SHA256 = hashlib.sha256(
    b"agent-evolve:durable-jsonl-outcome-adaptive-phase-journal:v1;"
    ...
    b"duplicate-receipt=fail-closed"
).hexdigest()
...
class DurableJsonlOutcomeAdaptivePhaseCommitter:
    """Append and fsync every causal boundary before execution continues."""

    path: Path
    committer_id: str = field(
        init=False,
        default=DURABLE_OUTCOME_ADAPTIVE_PHASE_JOURNAL_ID,
    )
    committer_version: int = field(
        init=False,
        default=DURABLE_OUTCOME_ADAPTIVE_PHASE_JOURNAL_VERSION,
    )
    definition_sha256: str = field(
        init=False,
        default=DURABLE_OUTCOME_ADAPTIVE_PHASE_JOURNAL_DEFINITION_SHA256,
    )
    ...
    def __post_init__(self) -> None:
        ...
        for line_number, raw in enumerate(
            ...
            try:
                ...
                raise ValueError(
                    f"invalid adaptive phase journal line {line_number}"
                ) from error
            if (
    ...
    async def commit(
        ...
        if type(receipt) is not OutcomeAdaptiveResidualPhaseReceipt:
        ...
        ack = OutcomeAdaptiveResidualPhaseCommitAck(
