...
def test_the_floor_binds_on_an_excluded_value_and_lands_exactly_on_it():
    ...
    assert values == (8, 16)
    assert weights[1] == pytest.approx(PRIOR_FLOOR)
...
def test_a_value_the_proposal_omitted_is_folded_back_in_at_the_floor():
    ...
    assert min(weights) == pytest.approx(PRIOR_FLOOR)
...
def test_flooring_only_ever_flattens_so_a_concentration_cap_survives():
    ...
    for weights in [(9.0, 1.0), (1.0, 0.0), (5.0, 4.0, 0.0), (1.0,) * 4]:
        floored = floor_weights(range(len(weights)), weights)[1]
...
def test_the_proposals_own_ranking_survives_the_floor():
    ...
    assert min(weights) >= PRIOR_FLOOR - 1e-12
...
def test_the_admitted_prior_keeps_the_floor_on_every_declared_value():
    ...
    assert min(width_weights) == pytest.approx(PRIOR_FLOOR)

    depth_values, depth_weights = prior.weighted["depth"]
    assert depth_values == (256, 512, 1024)
    assert min(depth_weights) == pytest.approx(PRIOR_FLOOR)
...
def test_prior_floor_zero_restores_the_admitted_prior_byte_for_byte():
    reply = json.dumps({"weights": {
        "width": {"values": [8], "weights": [1]},
        "depth": {"values": [256, 512], "weights": [4, 1]}}})
    prior = _proposer(reply, prior_floor=0.0)(_attr(), Arch)
...
def test_the_statistical_prior_floors_what_a_wide_screen_would_erase():
    ...
    # was making.
    prior = statistical_weighted_prior(_attr(), Arch)
    for _name, (_values, weights) in prior.weighted.items():
        shares = [w / sum(weights) for w in weights]
        assert min(shares) >= min(PRIOR_FLOOR, 1.0 / len(shares)) - 1e-12


def test_the_statistical_prior_at_floor_zero_is_the_pre_floor_arithmetic():
    attr = _attr()
    floored = statistical_weighted_prior(attr, Arch)
    raw = statistical_weighted_prior(attr, Arch, prior_floor=0.0)
